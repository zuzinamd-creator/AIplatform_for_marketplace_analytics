    ...
    <div className="space-y-6">
      ...
      <Card className="p-5">
        ...
        <dl className="mt-3 grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
          ...
          <div>
            <dt className="text-xs text-slate-400">Tenant ID</dt>
            <dd className="font-mono text-xs">{user?.id ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-400">Рабочее пространство</dt>
            <dd>{profile.workspace_name}</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-400">Режим продукта</dt>
            <dd>
              <StatusBadge tone="info">{settings.product_mode}</StatusBadge>
            </dd>
          </div>
          <div>
            <dt className="text-xs text-slate-400">API base</dt>
            <dd className="font-mono text-xs">{import.meta.env.VITE_API_BASE_URL ?? "http://localhost:8080"}</dd>
          </div>
          <div>
            <dt className="text-xs text-slate-400">Версия фронтенда</dt>
      ...
      <Card className="p-5">
        ...
        {notifications.length === 0 ? (
          <div className="mt-3 text-sm text-slate-400">Уведомлений пока нет.</div>
        ) : (
          <ul className="mt-3 space-y-2">
            {notifications.slice(0, 15).map((n) => (
              <li
                key={n.id}
                className={`rounded-lg border border-slate-800/70 p-3 text-sm ${n.read ? "opacity-60" : ""}`}
              >
                <div className="flex items-center justify-between gap-2">
                  <StatusBadge tone={n.tone}>{n.title}</StatusBadge>
                  <span className="text-[10px] text-slate-500">{n.created_at.slice(0, 19)}</span>
                </div>
                {n.detail ? <div className="mt-1 text-xs text-slate-300">{n.detail}</div> : null}
                <div className="mt-2 flex gap-2">
                  ...
                  {n.href ? (
                    <Link to={n.href} className="text-xs text-sky-300 hover:underline">
                      Открыть
                    </Link>
                  ) : null}
                </div>
      ...
      <Card className="p-5">
        ...
        <div className="mt-1 text-xs text-slate-400">События usage сохраняются в этом браузере для диагностики MVP.</div>
        {usage.length === 0 ? (
          <div className="mt-3 text-sm text-slate-400">Активность пока не зафиксирована.</div>
        ) : (
          <pre className="mt-3 max-h-64 overflow-auto text-[11px] text-slate-300">
