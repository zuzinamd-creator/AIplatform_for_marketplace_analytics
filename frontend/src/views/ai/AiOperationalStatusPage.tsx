  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <div className="text-sm text-slate-300">Degraded mode banners and confidence indicators should be visible to sellers.</div>
      </div>
      ...
      {q.isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          ...
          <Card className="p-5">
            ...
            <div className="mt-1 text-xs text-slate-400">Runs total: {String(r.runs_total ?? "n/a")}</div>
          </Card>
          <Card className="p-5">
            ...
            <div className="mt-1 text-xs text-slate-400">Avg confidence: {String(r.avg_confidence ?? "n/a")}</div>
          </Card>

          <Card className="p-5 md:col-span-3">
            <div className="text-sm font-semibold">Raw status payload</div>
            <pre className="mt-3 overflow-auto text-[11px] text-slate-300">{JSON.stringify(r, null, 2)}</pre>
          </Card>
        </div>
      ) : (
