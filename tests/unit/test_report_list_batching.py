"""Unit tests for report period columns on list API."""
...
@pytest.mark.asyncio
async def test_list_reports_does_not_scan_normalized_rows() -> None:
    user_id = uuid.uuid4()
    report_id = uuid.uuid4()
    report = _report(report_id, user_id)
    ...
    with patch.object(service, "_rls_transaction", return_value=tx_mock):
        with patch.object(service, "_latest_jobs_for_reports", AsyncMock(return_value={})):
            with patch(
                "app.domain.reports.period_queries.fetch_sale_period_bounds_for_reports",
                AsyncMock(),
            ) as nrr_mock:
                db.execute = AsyncMock(
                    return_value=MagicMock(
                        scalars=MagicMock(return_value=MagicMock(all=MagicMock(return_value=[report])))
                    )
                )
                with patch(
                    "app.services.report_service.report_to_response",
                    return_value=MagicMock(id=report.id),
                ):
                    await service.list_reports()

    nrr_mock.assert_not_called()
