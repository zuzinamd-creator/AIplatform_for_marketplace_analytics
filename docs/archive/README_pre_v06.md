...
- Детерминированная финансовая аналитика по периодам (выручка/прибыль/маржа/возвраты/выплаты) — `/api/v1/analytics/*`, dashboard, economics UI.
...
- AI advisory‑рекомендации: Period Intelligence (governed signals, 10 analysts, business coverage, exec summary v2) — §35.
- Персистентный workflow продавца: заметки, напоминания, история действий.
- Password recovery: forgot/reset password endpoints.
...
- **Period Intelligence Pipeline** (на отчёт): governed signals → deep period insights → 10 domain analysts → executive aggregator → quality/fatigue → persist.
- Каждая рекомендация (Period Intelligence) включает: `business_coverage`, executive summary v2, root cause confidence, actionable action plan (`lineage.insight_engine_version: coverage_v1`).
- Возвращает advisory‑рекомендации с:
  - confidence
  - evidence refs
  - limitations
  - пояснением “почему AI может ошибаться”
- Качество измеряется скриптами audit (§35); production path — `AIIntelligenceEngine`, Operating Director — scaffold only (§35.3).
...
- UI: `/app/ai/recommendations` / детали рекомендации (trust + limitations + business coverage + история действий)
- Качество и migration audit: **§35**
...
| `logistics_high_share` | Logistics | logistics share above threshold | logistics_share_pct | high | Review logistics-heavy SKUs |
| `logistics_share_growth` | Logistics | logistics share grew vs compare | logistics_share_pct | medium | Check delivery mix / returns |
| `returns_high_rate` | Returns | return rate above threshold | return_rate_pct | high | Inspect SKU return leaders |
| `returns_rate_growth` | Returns | returns grew vs compare | return_rate_pct | medium | Validate listing / quality |
| `revenue_drop` / `revenue_growth` | Revenue change | period-over-period revenue delta | total_revenue | high/medium | Review price, stock, promo mix |
| `profit_drop` | Revenue change | period-over-period profit delta | total_profit | high | Confirm COGS coverage |
| `concentration_top1_risk` | Concentration | top-1 SKU > threshold share | top_sku_concentration | high | Diversify revenue mix |
| `concentration_top3_risk` | Concentration | top-3 SKU > threshold share | top_sku_concentration | medium | Reduce dependency on few SKUs |
| `deep_period_insights.*` | Deep insights | unprofitable SKU, commission-heavy, missing cost | deep_insights | varies | See `app/ai/deep/period_insights.py` |

Полный каталог Phase 6.1: `app/ai/quality/recommendation_audit.py` (`BUSINESS_CASE_CATALOG`, `ANALYST_RULE_CATALOG`).
...
#### Ограничения MVP (остаточные)

- **Logistics / returns / revenue change / concentration** — deterministic rules (Phase 6.1); commission-heavy / unprofitable SKU — через `deep_period_insights`
- **Реклама, CTR, остатки, налоги, opex** — analysts подготовлены, но без данных не запускаются (см. Business Coverage + §35.3)
- Profit/margin скрыты при cost coverage < 100% (trust gating) → `sales_low_margin` может не сработать
- LLM-заголовки могут повторять KPI дашборда (Dashboard Echo) — известный gap, см. §35.2
...
- **Analytics exploration**: dashboard + economics wired to `/api/v1/analytics/*` (§26); dedicated Weekly Analysis UI still partial
...
### Analytics read layer (implemented — see §26)

Seller KPI read APIs реализованы в `app/api/analytics.py` и используются dashboard / economics UI. UX-1 формулировка «Missing APIs» **устарела** после ANALYTICS-1.

Оставшиеся UX gaps: Weekly Analysis UI (period compare + ABC + inventory risk в одном экране), SKU mapping CRUD, server-side tenant settings.
...
|-----------|-------|--------|
| MVP readiness | **7.0 / 10** | Upload + KPI dashboard + AI advisory; weekly UI + SKU mapping gaps |
| Production readiness | **6.5 / 10** | Backend mature; server-side tenant settings pending |
| AI usefulness (subjective UX-3) | 6.0 / 10 | Feedback loop + transparency; см. §35.2 для audit metrics |
| UX maturity | 6.5 / 10 | Onboarding + trust banners; reduce raw JSON exposure |

**AI quality (Phase 6.2.1, post-migration audit):** Actionable **100%**, AI Readiness **61.6**, **NO-GO** — см. §35.2.

Historical UX-3 baseline: [docs/product/mvp_readiness.md](docs/product/mvp_readiness.md) (частично устарел; KPI APIs и password recovery уже реализованы).
...
1. Weekly Analysis UI (period compare, ABC, inventory risk — APIs готовы)
2. SKU mapping CRUD + UI
3. AI Dashboard Echo fix + Seller Usefulness ≥80 (§35.2)
4. Server-side tenant settings
5. Operating Director production wiring (§35.3, optional)
...
This phase improves AI **usefulness, clarity, actionability, and trust** without adding orchestration, agents, or infrastructure. Extended by **Phase 6** (§35): business coverage, audit metrics, migration pipeline.
...
- **Business Coverage Engine** (Phase 6.2): `action_plan.business_coverage`, executive summary v2, root cause confidence, advertising warning
- **Quality audit** (Phase 6.1–6.2.1): `app/ai/quality/recommendation_audit.py`, scripts §35.2
...
- **Explainability UX upgrades**:
  ...
  - Business Coverage % badge on recommendation detail
...
- `docs/ai/operating_director_architecture.md` — Phase 6.3 scaffold (§35.3)
...
**AI quality benchmark (Phase 6.2.1, deterministic audit):** Actionable 100%, Seller Usefulness 42.5, Dashboard Echo 25%, AI Readiness 61.6 → **NO-GO** (§35.2). Simulation scores above ≠ production AI gate.

### MVP blockers (refined)

1. Weekly Analysis UI (period compare, ABC, inventory risk)
2. SKU mapping CRUD + UI
3. AI quality gate: Dashboard Echo ≤10%, Seller Usefulness ≥80, AI Readiness ≥80 (§35.2)
4. Server-side tenant settings
5. Costs-import nudge when margin KPIs shown without costs

**Resolved since UX-3:** seller KPI read APIs (§26), password recovery (`POST /auth/forgot-password`, `/auth/reset-password`).
...
AIInsightInputDTO (deterministic KPIs)
  ...
  → 10× Domain Analyst (structured JSON findings)
...
| Logistics | Phase 6.1 — `logistics_analyst.*` |
| Returns | Phase 6.1 — `returns_analyst.*` |
| Revenue change | Phase 6.1 — `revenue_change_analyst.*` |
| Concentration | Phase 6.1 — `concentration_analyst.*` |
...
- `docs/ai/operating_director_architecture.md` — Phase 6.3 target architecture (§35.3)
...
| `scripts/backfill_ai_recommendations.py` | Backfill AI recs per finance report (`--all-reports` force) |
| `scripts/ai_recommendation_quality_audit.py` | Phase 6.1/6.2 quality audit (§35.2) |
| `scripts/phase_621_migration_audit.py` | Full migration + before/after audit (§35.2.1) |
...
## 35. AI Quality, Coverage & Operating Director (Phase 6)

Phase 6 improves **measurable** recommendation quality without replacing the production `AIIntelligenceEngine` (Operating Director — scaffold only until Iteration 3).

### 35.1 Phase 6.1 — Deterministic analysts & quality audit

**Added analysts** (10 total in `app/ai/analysts/orchestrator.py`):

- Logistics, Returns, Revenue change, Concentration (+ existing Sales, Ads, Funnel, Inventory, Marketplace, Anomaly)
- Governed signals: `app/ai/analysts/governed_signals.py`
- Deep period insights: `app/ai/deep/period_insights.py`, `period_causes.py`

**Audit module:** `app/ai/quality/recommendation_audit.py` — classification (Insight / Dashboard Echo / False Positive / Data Quality Warning), echo detection, actionability, MVP AI score, GO/NO-GO.

### 35.2 Phase 6.2 — Business Coverage & executive summary v2

**Business Coverage V1** (`app/ai/coverage/business_coverage.py`):

- Blocks: sales, MP economics, COGS, ads, inventory, tax, opex (weights → `business_coverage_score`)
- Persisted in `action_plan.business_coverage`, `executive_summary_v2`, `root_cause_confidence`, `analysis_limitations`, `advertising_warning`
- `lineage.insight_engine_version: coverage_v1`
- UI: coverage % badge on recommendation detail

**Period Intelligence Pipeline** (per finance report via `AIService.run_intelligence`):

```text
report → governed signals + deep insights → domain analysts → executive → quality/fatigue → persist
```
...
**GO/NO-GO criteria** (`evaluate_go_no_go`):

| Check | MVP threshold |
|-------|---------------|
| Actionable Rate | ≥ 80% |
| Seller Usefulness | ≥ 80 |
| Dashboard Echo | ≤ 10% |
| AI Readiness Score | ≥ 80 |

**Post-migration pilot audit** (4 rec, Period Intelligence only, 2026-06-07):

| Metric | Value | Pass |
|--------|-------|------|
| Actionable Rate | 100% | ✅ |
| Seller Usefulness | 42.5 | ❌ |
| Dashboard Echo | 25% | ❌ |
...
Report: `reports/phase_621_migration_audit.json`

**Known gaps:** LLM titles «Общий доход…» → Dashboard Echo; missing ads/CTR/inventory data caps Seller Usefulness; executive summary v2 in summary text but no dedicated UI section.

### 35.3 Phase 6.3 — Operating Director (scaffold, not production)

Target architecture (strangler-fig, not wired to production yet):

```text
L0 Data Quality Auditor → L1 Domain Experts → L2 Cross-Domain Analysts → L3 Executive Director → Seller Report
```

Code: `app/ai/director/` — `coverage_v2.py`, `data_quality_auditor.py`, `domain_experts.py`, `cross_domain.py`, `executive_director.py`, `pipeline.py`

Full design + migration plan (6–7 iterations): [docs/ai/operating_director_architecture.md](docs/ai/operating_director_architecture.md)

**Status:** shadow-run / feature flag planned; legacy `AIIntelligenceEngine` remains production path.

### Phase 6 docs & tests

- `docs/ai/operating_director_architecture.md`
- `tests/unit/test_business_coverage.py`
- `tests/unit/test_operating_director_scaffold.py`
- `tests/unit/test_ai_scenario_recommendations.py`

```bash
pytest tests/unit/test_business_coverage.py tests/unit/test_operating_director_scaffold.py -q
```
